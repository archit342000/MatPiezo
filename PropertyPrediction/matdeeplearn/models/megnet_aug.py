...
class MEGNet_AUG(torch.nn.Module):
    def __init__(
        self,
        data,
        dim1=64,
        ...
        **kwargs
        ...
        super(MEGNet_AUG, self).__init__()
        ...
        if post_fc_count > 0:
            ...
            for i in range(post_fc_count):
                if i == 0:
                    ...
                    if self.pool_order == "early" and self.pool == "set2set":
                        ...
                        lin = torch.nn.Linear(post_fc_dim * 3+32+7, dim2)
            ...
            if self.pool_order == "early" and self.pool == "set2set":
                ...
                self.lin_out = torch.nn.Linear(post_fc_dim * 3+32+7, output_dim)              
            else:
    ...
    def forward(self, data):
        ...
        # x_aug = torch.cat([data.pge, data.cse], dim=1)          
        device = out.get_device()
        # x_aug = torch.zeros((out.shape[0], 32+7)).to(device)
        ...
        if self.pool_order == "early":
            ...
            out = torch.cat([out, data.pge], dim=1)
            out = torch.cat([out, data.cse], dim=1)
            # x_aug = torch.zeros((out.shape[0], 32+7)).to(device)
            # out = torch.cat([out, x_aug], dim=1)
