...
# CGCNN
class GATGNN_AUG(torch.nn.Module):
    def __init__(
        self,
        data,
        dim1=64,
        ...
        **kwargs
        ...
        super(GATGNN_AUG, self).__init__()
        ...
        if post_fc_count > 0:
            ...
            for i in range(post_fc_count):
                if i == 0:
                    self.post_lin_list.append(torch.nn.Linear(post_fc_dim+32+7, dim2))
                else:
                    self.post_lin_list.append(torch.nn.Linear(dim2, dim2))
