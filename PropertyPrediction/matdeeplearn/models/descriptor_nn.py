...
class SOAP(torch.nn.Module):
    def __init__(self, data, dim1, fc_count,  **kwargs):
        ...
        self.bn_list = torch.nn.ModuleList(
            [BatchNorm1d(dim1) for i in range(fc_count)]
        )
    ...
    def forward(self, data):
        ...
        out = F.silu(self.bn1(self.lin1(data.extra_features_SOAP.view(-1, data[0].extra_features_SOAP.shape[-1]))))
        for i in range(len(self.lin_list)):
            out=out+F.silu(self.bn_list[i](self.lin_list[i](out)))
        out = self.lin2(out)
        if out.shape[1] == 1:
            return out.view(-1)
        else:
