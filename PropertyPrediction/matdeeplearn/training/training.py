...
def train(model, optimizer, loader, loss_method, rank):
    ...
    count = 0
    for data in loader:
        batch_size=len(data.structure_id)
        data.y=data.y.view(batch_size, -1)
        data = data.to(rank)
        optimizer.zero_grad()
        output = model(data)
...
#         data=data.to(rank)
#         optimizer.zero_grad()
#         output = model(data)    # shape = (num_nodes_batch, 2)
#         pos = torch.ones([output.shape[0], 1], dtype=torch.long).to(rank)    # shape = (num_nodes_batch, 1)
#         neg = torch.zeros([output.shape[0], 1], dtype=torch.long).to(rank)    # shape = (num_nodes_batch, 1)
#         out = torch.cat([pos, neg], dim=1)    # shape = (num_nodes_batch, 2)
#         loss = getattr(F, loss_method)(output, out)
...
def predict(dataset, loss, job_parameters=None, processing_args=None):
    ...
    for data in loader:
        ...
        with torch.no_grad():
            ...
            if processing_args["targets"] == "True":
                loss_all = loss_all + (getattr(F, loss)(output, data.y).item() * output.size(0))
            ...
            if count == 0:
                ...
                predictions = np.concatenate(
                    (predictions, output.data.cpu().numpy()), axis=0
                )

                if processing_args["targets"] == "True":
                    targets = np.concatenate((targets, data.y.cpu().numpy()), axis=0)
    ...
    for i in range(len(predictions)):
        if(len(predictions[i]))>1:
            predictions[i] = ','.join(map(str, predictions[i]))
    ...
    if processing_args["targets"] == "True":
        ...
        for i in range(len(targets)):
            if(len(targets[i]))>1:
                targets[i] = ','.join(map(str, targets[i]))

    if processing_args["targets"] == "True":
        test_out = np.column_stack((ids, targets, predictions))
    else:
        test_out = np.column_stack((ids, predictions))
...
def train_CV(
    ...
    print("Fixed seeds for reproducibility - seed: ", seed)
    ...
    for index in range(0, len(cv_dataset)):
        ...
        torch.manual_seed(job_parameters["seed"])
        ##Start training
        model = trainer(
            rank,
            world_size,
